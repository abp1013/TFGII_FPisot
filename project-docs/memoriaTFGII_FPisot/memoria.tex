...
\begin{abstract}
	Eco City Tours is a mobile application \textbf{developed with Flutter} that suggests tourist routes to users. The application focuses on promoting non-motorized routes, optimized for cyclists and pedestrians, connecting points of interest with the goal of fostering sustainable mobility. The routes are generated using technologies based on \acrfull{gis}. The \acrfull{poie} are enriched with detailed information about tourist spots through artificial intelligence, consulting \acrfull{llme} via natural language queries.	
